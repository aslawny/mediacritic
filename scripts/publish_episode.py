...
def main():
    ...
    # On repere l'entree existante par son URL, jamais par la ligne entiere :
    # une relance apres changement de titre ne retrouvait pas l'ancienne ligne
    # et en ajoutait une seconde. L'accueil a ainsi affiche deux fois
    # l'episode 43, les deux entrees pointant vers la meme page.
    existantes = [l for l in idx.split("\n")
                  if f'href="episodes/{page}"' in l and "<li>" in l]
    if existantes:
        for doublon in existantes[1:]:
            idx = idx.replace("\n" + doublon, "", 1)
        idx = idx.replace(existantes[0], li, 1)   # titre eventuellement modifie
    else:
